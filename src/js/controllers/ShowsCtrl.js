...
function($scope, $rootScope, $state, $firebase, $filter, simpleLogin, leafletData, Player) {
            ...
            angular.extend($scope, { // Map data
                geojson: {
                    ...
                    onEachFeature: function (feature, layer) {
                        layer.bindPopup(feature.properties.artist + '&nbsp;&#64;&nbsp;' + feature.properties.venue_name);
 ...
 // Experimental Player with service
    ...
    angular.element('.list-group-item').removeClass('playing');

      if (Player.currentTrack && Player.currentTrack.properties.id) {
        angular.element('.list-group-item' + Player.currentTrack.properties.id).addClass('playing');
      }
    });
    ...
    defaults: {
        tileLayer: 'https://api.tiles.mapbox.com/v4/mapbox.streets/{z}/{x}/{y}.png?access_token=pk.eyJ1Ijoia3Blbm5lbGwiLCJhIjoid3QxaFNMTSJ9.5HjAo-fmMghcHGeCMOMheg',
        maxZoom: 16,
        path: {
            weight: 2,
            color: '#800000',
